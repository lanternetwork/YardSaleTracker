...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        ALTER TABLE sales ADD COLUMN privacy_mode text DEFAULT 'exact' 
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        ALTER TABLE sales ADD COLUMN status text DEFAULT 'draft' 
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        ALTER TABLE sales ADD COLUMN source_id text;
    END IF;

    -- Timestamp fields
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'sales' AND column_name = 'first_seen_at') THEN
        ALTER TABLE sales ADD COLUMN first_seen_at timestamptz DEFAULT now();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'sales' AND column_name = 'last_seen_at') THEN
        ALTER TABLE sales ADD COLUMN last_seen_at timestamptz DEFAULT now();
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS negative_matches (
    ...
    sale_id_a uuid NOT NULL REFERENCES sales(id) ON DELETE CASCADE,
    sale_id_b uuid NOT NULL REFERENCES sales(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_sales_title_trgm ON sales USING gin (title gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_sales_date_start ON sales (date_start);
CREATE INDEX IF NOT EXISTS idx_sales_last_seen_at ON sales (last_seen_at DESC);
CREATE INDEX IF NOT EXISTS idx_sales_status ON sales (status);

-- Partial unique index for source tracking
CREATE UNIQUE INDEX IF NOT EXISTS idx_sales_source_unique 
    ON sales (source, source_id) 
...
COMMENT ON COLUMN sales.privacy_mode IS 'Controls coordinate masking: exact=show precise location, block_until_24h=mask until 24h before start';
COMMENT ON COLUMN sales.status IS 'Sale lifecycle: draft=unpublished, published=live, hidden=manually hidden, auto_hidden=system hidden';
