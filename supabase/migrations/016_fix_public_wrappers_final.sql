-- Fix public wrappers with correct column mappings for all tables
...
-- Create public view for profiles_v2 (using only existing columns)
CREATE OR REPLACE VIEW public.profiles_v2 AS
SELECT
  id as user_id,
  display_name,
  COALESCE(avatar_url, '') as avatar_url,
  ''::TEXT as home_zip,  -- Hardcoded since column doesn't exist
  '{}'::JSONB as preferences,  -- Hardcoded since column doesn't exist
  created_at,
  created_at as updated_at  -- Use created_at since updated_at doesn't exist
FROM profiles;
