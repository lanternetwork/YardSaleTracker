...
describe('URL Normalization - No Mock Data', () => {
  const feedUrl = 'https://louisville.craigslist.org/search/garage-sale?format=rss'

  it('should keep absolute craigslist URLs unchanged', () => {
    const absoluteUrl = 'https://louisville.craigslist.org/garage-sale/1234567890.html'
  ...
  it('should resolve relative URLs against feed origin', () => {
    ...
    expect(result).toBe('https://louisville.craigslist.org/garage-sale/1234567890.html')
  })
  ...
  it('should reject HTTP URLs (only HTTPS allowed)', () => {
    const httpUrl = 'http://louisville.craigslist.org/garage-sale/123.html'
    const result = normalizeUrl(httpUrl, feedUrl)
    expect(result).toBeNull()
  })
  ...
  it('should preserve query parameters and fragments', () => {
    const urlWithParams = 'https://louisville.craigslist.org/garage-sale/123.html?utm_source=test#section'
  ...
  it('should handle complex relative paths', () => {
    ...
    expect(result).toBe('https://louisville.craigslist.org/garage-sale/123.html')
  })

  it('should handle different feed origins', () => {
    const seattleFeed = 'https://seattle.craigslist.org/search/garage-sale?format=rss'
    const relativeUrl = '/garage-sale/123.html'
    const result = normalizeUrl(relativeUrl, seattleFeed)
    expect(result).toBe('https://seattle.craigslist.org/garage-sale/123.html')
  })
})
