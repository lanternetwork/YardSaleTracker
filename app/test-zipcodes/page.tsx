...
export default function TestZipcodesPage() {
  ...
  const seedZipcodes = async (dryRun = false) => {
    setSeedLoading(true)
    setSeedResult(null)
    
    try {
      const url = dryRun 
        ? '/api/admin/seed/zipcodes?dryRun=true'
        : '/api/admin/seed/zipcodes'
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="mb-6 space-y-4">
        <div className="flex gap-4 flex-wrap">
          ...
          <button
            onClick={() => seedZipcodes(true)}
            disabled={seedLoading}
            className="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600 disabled:opacity-50"
          >
            {seedLoading ? 'Analyzing...' : 'Dry Run (Count Only)'}
          </button>
          
          <button
            onClick={() => seedZipcodes(false)}
            disabled={seedLoading}
